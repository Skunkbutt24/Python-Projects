logo='''
                                          88                          
                                    ,d    ""                          
                                    88                                
,adPPYYba, 88       88  ,adPPYba, MM88MMM 88  ,adPPYba,  8b,dPPYba,   
""     `Y8 88       88 a8"     ""   88    88 a8"     "8a 88P'   `"8a  
,adPPPPP88 88       88 8b           88    88 8b       d8 88       88  
88,    ,88 "8a,   ,a88 "8a,   ,aa   88,   88 "8a,   ,a8" 88       88  
`"8bbdP"Y8  `"YbbdP'Y8  `"Ybbd8"'   "Y888 88  `"YbbdP"'  88       88

    '''   
print(logo)
print("Welcome to my virtual bidding program.")
people = int(input("How many people are there?\n"))
names = []
bids = []
for x in range(1,people+1):
    print("\n"*100)
    names.append(input(f"Bidder number {x} name:\n"))
    bids.append(int(input("What's your bid?\n ")))
winner_name = names[bids.index(max(bids))]
print(f"Highest bid: {max(bids)} by {winner_name}")

    